...
void parseLidarFromFile(std::string str, long long &first_timestamp, long long &timestamp, std::vector<long> &vec);
...
int main (int argc, char** argv)
{
    ...
    double max_diff = 0.0, min_diff = 1.0, mean_diff, diff;
    ...
    long long current_ts = 0, previous_lidar_ts = 0, first_timestamp = 0;
    ...
    {		
        ...
        parseLidarFromFile(lidar_line, first_timestamp, current_ts, lidar_readings); 
        ...
        {                   
            ...
            diff = (current_ts-previous_lidar_ts)*pow(10,-3);
            ...
            mean_diff = (mean_diff*(n_done-1) + diff)/n_done;
            ROS_INFO_STREAM("prev: " << previous_lidar_ts << "\tcur: " << current_ts << "\tdiff: " << diff << "\tmin: " << min_diff << "\tmax: " << max_diff << "\tmean: " << mean_diff);
        }          
    }
    bag.close();
    ROS_INFO_STREAM("Done. LiDAR measurements saved to \n\n" << bag_name+".bag\n\n");
...
void parseLidarFromFile(std::string str, long long &first_timestamp, long long &timestamp, std::vector<long> &vec)
{
    ...
    if (aux.size()==1083)
    {
        ...
        {
            first_timestamp = (h*3600 + m*60 + s)*1000;
        }
        ROS_INFO_STREAM("\nh: " << h << " m: " << m << " s: " << s << " ms: " << ms);
